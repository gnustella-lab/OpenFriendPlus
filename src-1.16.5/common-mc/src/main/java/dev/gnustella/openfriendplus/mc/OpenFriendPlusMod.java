...
public final class OpenFriendPlusMod {
    ...
    public static final Logger LOG = LogManager.getLogger("openfriendplus");
    ...
    private static void handOffMinecraftSessionOrSignIn(IpcClient ipc, FriendsState s) {
        ...
        if (token != null && !token.isEmpty() && profileId != null) {
            ...
            LOG.info("OpenFriend Plus: handing Minecraft session to Core (user={})", privacy.maskName(name));
            ...
            LOG.info("OpenFriend Plus: no Minecraft session available; using device-code sign-in (expected profileId={})", privacy.maskUuid(profileId));
            triggerDeviceCodeSignIn(ipc, s, profileId);
        }
    }
