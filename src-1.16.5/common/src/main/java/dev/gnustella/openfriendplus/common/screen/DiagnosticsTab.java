...
public final class DiagnosticsTab implements FriendsOverlayScreen.Tab {
    ...
    @Override public String label() { return Lang.tr(TranslationKey.TAB_DIAGNOSTICS, "Diagnostics"); }
    ...
    private final class BodyPanel extends UPanel {
        ...
        private final UButton refreshBtn = new UButton(Lang.tr("openfriendplus.action.refresh", "Refresh"), () -> { if (refresh != null) refresh.run(); }).setStyle(UButton.Style.SUBTLE);
        private final UButton copyBtn = new UButton(Lang.tr("openfriendplus.action.copy", "Copy"), this::copyDiagnostics).setStyle(UButton.Style.SUBTLE);
        ...
        private void copyDiagnostics() {
            try {
                java.awt.Toolkit.getDefaultToolkit()
                        .getSystemClipboard()
                        .setContents(new java.awt.datatransfer.StringSelection(diagnostics.copyText()), null);
                notice.success("Diagnostics copied", "Diagnostic text is now on the clipboard.");
            } catch (Throwable t) {
                notice.warn("Clipboard unavailable", "Diagnostics could not be copied automatically.");
            }
        }
    }
