...
public final class FriendsController {
    ...
    public dev.gnustella.openfriendplus.common.notice.NoticeSink notice() { return notice; }
        ...
        return new FriendEntry.Actions() {
            @Override public void onJoin(Friend f) {
                ...
                ipc.requestAsync("join.start", params).whenComplete((result, err) -> {
                    if (err != null) {
                        boolean alreadyRunning = err instanceof dev.gnustella.openfriendplus.common.ipc.IpcException
                                && ((dev.gnustella.openfriendplus.common.ipc.IpcException) err).isAlreadyRunning();
                        if (alreadyRunning) {
                            if (joinLauncher != null) joinLauncher.connectToLocalAddress(listen);
                            return;
                        }
                        java.util.logging.Logger.getLogger("openfriendplus.join").warning(
