...
public final class PrivacyFormatter {
    ...
    public String maskName(String name) {
        if (!shouldMaskNames() || name == null || name.length() <= 2) return name;
    ...
    public String maskUuid(String uuid) {
        if (!shouldMaskUuid() || uuid == null || uuid.length() <= 8) return uuid;
        return uuid.substring(0, 8) + "-****";
    }

    public String maskAddress(String hostPort) {
        if (!shouldMaskSensitiveRuntimeData() || hostPort == null) return hostPort;
    ...
    public String maskDeviceCode(String code) {
        if (!shouldMaskSensitiveRuntimeData() || code == null || code.length() < 4) return code;
        return code.substring(0, Math.min(2, code.length())) + "**-****";
    }

    public boolean hidesIdentity() {
        return shouldMaskNames();
    }

    private boolean shouldMaskNames() {
        return config != null && (config.streamerMode || config.hideGamertags);
    }

    private boolean shouldMaskUuid() {
        return config != null && (config.streamerMode || config.hideProfileIdsInLogs);
    }

    private boolean shouldMaskSensitiveRuntimeData() {
        return config != null && config.streamerMode;
    }
}
